    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-5 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-3 group select-none">
          ...
          />
          <div>
            <div className="text-gray-900 font-semibold text-sm leading-none tracking-wide" style={{ fontFamily: "Sora, sans-serif" }}>Everyday Event</div>
          ...
          {links.map(({ label, href, isHash }) =>
            isHash ? (
              <a
                key={label}
                href={href}
                className="relative text-gray-600 hover:text-[#E31C25] text-[11px] tracking-[0.18em] uppercase transition-colors duration-300 group"
              >
                {label}
        ...
        <div className="hidden lg:flex items-center gap-5">
          <a href="tel:+919876543210" className="flex items-center gap-2 text-gray-500 hover:text-gray-800 text-[11px] transition-colors duration-300">
            <Phone size={11} />
        ...
        <button className="lg:hidden text-gray-600 hover:text-gray-900 transition-colors" onClick={() => setOpen(!open)} aria-label="Toggle menu">
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="lg:hidden overflow-hidden bg-white/95 backdrop-blur-2xl border-t border-gray-200"
          >
              ...
              {links.map(({ label, href, isHash }) =>
                isHash ? (
                  <a
                    key={label}
                    href={href}
                    className="text-gray-800 text-2xl font-semibold hover:text-[#E31C25] transition-colors duration-200"

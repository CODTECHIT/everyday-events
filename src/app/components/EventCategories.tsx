    ...
    <section id="about" className="py-28 bg-white">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <FadeUp>
          ...
          <h2
            className="font-bold text-gray-900 max-w-xl leading-[1.08]"
        ...
        <div className="mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          {cats.map(({ label, img, desc }, i) => (
            <FadeUp key={label} delay={i * 0.08}>
              <div className="group relative overflow-hidden bg-gray-50 border border-gray-200 cursor-pointer h-80 lg:h-[420px]">
                <img
                  src={img}
                  alt={label}
                ...
                <div className="absolute bottom-5 left-5 right-5">
                  ...
                  >{label}</h3>
                  <p className="text-white/80 text-[11px] mt-1 group-hover:text-white transition-colors duration-300">{desc}</p>
                </div>
              </div>
            </FadeUp>
          ))}

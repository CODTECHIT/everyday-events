      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 mb-14">
          <div className="col-span-2 lg:col-span-1">
            ...
            <p className="text-gray-700 text-xs leading-relaxed max-w-[220px]">
            ...
            <div className="flex items-center gap-4 mt-6">
              {([Instagram, Facebook, Twitter] as const).map((Icon, i) => (
                <a
                  key={i}
                  href="#"
                  className="text-gray-600 hover:text-[#E31C25] transition-colors duration-300"
          ...
          {footerLinks.map(({ title, links }) => (
            <div key={title}>
              <h4
                className="text-gray-900 font-semibold text-[10px] tracking-[0.2em] uppercase mb-5"
                style={{ fontFamily: "Sora, sans-serif" }}
              >{title}</h4>
              <ul className="space-y-3">
                {links.map(link => (
                  <li key={link}>
                    <a href="#" className="text-gray-700 hover:text-[#E31C25] transition-colors duration-300">
                      {link}
                    </a>
                  </li>
                ))}
        ...
        <div className="border-t border-gray-200 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-gray-600 text-[10px] tracking-wider">© 2024 Everyday Event Solutions. All rights reserved.</p>
          <p className="text-gray-600 text-[10px] tracking-wider">Noida, Delhi NCR • India</p>
        </div>
      </div>
    </footer>
  );
